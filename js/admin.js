...
if (courseForm) {
  courseForm.addEventListener('submit', async (e) => {
    ...
    const data = {
      ...
      short_description: document.getElementById('cfShortDesc').value.trim(),
      description: document.getElementById('cfDescription').value.trim(),
      syllabus: document.getElementById('cfSyllabus').value.trim(),
      duration: document.getElementById('cfDuration').value.trim(),
      eligibility: document.getElementById('cfEligibility').value.trim(),
      active: document.getElementById('cfActive').checked ? 1 : 0
    };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.getElementById('enquiriesTable')) {
    loadEnquiries();
    const sf = document.getElementById('enqStatusFilter');
    const si = document.getElementById('enqSearch');
    if (sf) sf.addEventListener('change', loadEnquiries);
    if (si) { let t; si.addEventListener('input', () => { clearTimeout(t); t = setTimeout(loadEnquiries, 300); }); }
  }
